...
def _classify_segment(
    ...
    if executable in _DELETE_COMMANDS and (
        ...
        return (
            CommandRisk.HARD_DENY,
            f"shell deletion is not an approved file operation: {executable}",
            hazards,
        )
    ...
    if executable in _POWER_SHELL:
        ...
        if "-command" in tokens:
            ...
            return (
                CommandRisk.REQUIRE_INTERVENTION,
                "nested PowerShell script requires approval",
                hazards,
            )
    ...
    if executable in {"python", "python3", "node", "perl", "ruby", "bash", "sh", "zsh"}:
        if any(token in {"-c", "-e", "--eval", "-command"} for token in tokens):
            ...
            return (
                CommandRisk.REQUIRE_INTERVENTION,
                "inline interpreter script requires approval",
                hazards,
            )
    ...
    return (
        CommandRisk.REQUIRE_INTERVENTION,
        f"unclassified command: {executable}",
        {"unknown_command"},
    )
...
def _risk_rank(risk: CommandRisk) -> int:
    return {CommandRisk.SAFE: 0, CommandRisk.REQUIRE_INTERVENTION: 1, CommandRisk.HARD_DENY: 2}[
        risk
    ]
