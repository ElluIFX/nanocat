...
- `ToolExecutionContext`：`turn_id`、`session_key`、`storage_scope`、`conversation`、`principal_id`、`state_hook`、`message_id`、`session`、`model`、`subagent_model`、`reasoning_effort`、`pulse_enabled`、`user_input`；`storage_scope` 绑定实际 session 文件生命周期，model/effort/pulse 使用 turn admission 时的快照，授权仍由 principal/conversation/turn/capability 决定。
...
- `WorkingFeed` 是对话内 thinking、tool、subagent、approval、progress、error 和 control 的统一组件；tool input/result 按 call ID 合并，stop 生命周期按稳定 node ID 合并。活动时展开并贴底跟随，用户上滚后暂停跟随，回到底部恢复；终态成功自动折叠并显示总耗时，失败与取消保持展开。Provider/turn error 展示脱敏后的 code、message 和恢复建议；steer、stop、interrupt、runtime shutdown 使用统一紧凑 control 卡，`turn.steer_applied`/`turn.steer_rejected` 提供显式终态，不从后续 thinking 推断。TODO、文件/搜索、terminal/SSH、network 和 subagent 使用同一 tool card shell，原始 input/output 保留在 Details；内部 `contentChars`/`artifactCount` telemetry 只参与结构处理。
...
- `/stop` 在任何 await 前建立 session stop 域、截取同 session queued 输入并取消 active/direct turn；全局 durability scheduler 跳过活动 stop 域，并在取得 session lock 后再次检查，域内新 ingress 由 stop owner 分类后继续入队或形成一次 terminal record。append-only turn ledger 保存已形成的 user/assistant/tool 消息，为尚未记录结果的 tool call 在下一 user/assistant 前补充结构化 cancelled result；terminal assistant 保存 `turn_control.kind`，projection 将主动 stop、一般 interruption 和 runtime shutdown 映射为对应 control feed。临时 stop requeue 与失败后的 pending durability 使用独立 owner；durability journal 保留原 terminal error/control 与 tool status，重启 replay 不改写其语义。未 drain steer 独立归属其原始 normal session；Web steer admission 释放 join 时若应用 deferred terminal，会补发对应 terminal activity，避免完成后仍显示 Running。工具轮数上限、Provider/application error、runtime interruption和空/pulse 输出均形成可见 terminal record；空闲 `/stop` 仅返回 control outbound，history 保持原状。
- Provider `finish_reason=error` 进入 failed turn；terminal record 保存脱敏且有界的 `turn_error`，Web SSE/activity 与 session fallback projection 共用同一错误包络。Conversation 客户端按 turn ID 合并 trajectory activity，使 error、steer 和 stop control 在刷新后保持同一 feed 语义。
...
- session 侧栏条目支持就地 rename 与二次确认 delete；session 顶栏三点菜单按 provider 分组切换 agent/subagent model，并提供 effort、pulse、compact 和 delete。model/effort/pulse 写入有字段级 saving/applied 反馈，忙碌 turn 结束后再允许变更，下一 turn 在 admission 时捕获一致快照。
...
- 2026-08-23 本地隔离实例已验证密码登录、错误密码 401、第四次失败 429、`Retry-After`、锁定期间正确密码回退、CSRF、session CRUD、conversation/trajectory projection、command、上传、artifact metadata/Range、SSE `turn.queued`、exact turn cancel、删除后 404 和统一关闭；Playwright Chromium 桌面/平板/移动端真实 API 与视觉回归 27/27 已覆盖当时响应式基线。2026-08-24 本轮状态/Feed 改造在最终 UI 微调前完成后端离线回归 100/100、Vitest 22/22、TypeScript/Vite build、Ruff、compileall 与 diff check；隔离 runtime 可启动，当前环境无可连接浏览器，因此最终 UI 微调按用户要求交由人工运行判断，未重复静态检查或浏览器回归。
