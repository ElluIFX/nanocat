...
- `CommandDispatcher` 为每条命令生成独立 control outbound，保留 event/correlation/request/principal 关联；TUI Action Center、`process_direct()` 和渠道 slash 输入复用同一执行器。
...
- 设置页内容位于可滚动的 `VerticalScroll` 中；模型目录管理是 Settings 内的居中二级面板（`ModelManagerScreen`/`ModelEditScreen`），经 `model.add`/`model.update`/`model.remove` 结构化动作以稳定 model 字符串操作（不用列表序号）；`model.update` 会让 agent/subagent/assistant/vision/compaction 槽位引用跟随改名，`model.remove` 拒绝删除仍被槽位引用或仅剩的条目。Screen 上没有 `push_screen`，必须由 `self.app.push_screen` 调起二级 Screen。
- 日志面板是默认隐藏的右侧抽屉：缓冲 2000 条、渲染上限 500、支持文本/级别过滤、暂停、清空（仅视图）；file sink 不受影响。`display:none` 的 RichLog region 宽度为 0，重新打开后必须等 relayout 完成再渲染（`call_after_refresh` + 宽度守卫重试），否则内容按宽度 0 换行且不重排，表现为二次打开空白。
- 右侧是 `#right-pane` 状态容器：默认显示 `StatusPanel`（session/model/effort/context 条形图/runtime busy/approval 状态，snapshot 驱动 + 15s 周期刷新 + busy tick 帧动画），打开日志时整体切换为日志抽屉（`#right-pane.-logs`）；窄屏下右侧容器默认隐藏，日志打开时以 overlay 浮层呈现。抽屉关闭期间新日志在 header Logs 按钮上显示 `-notify` 提示点。
- 退出手势是双击 Ctrl+C（窗口 1.5s，第一次只在 notify 中提示），Ctrl+Q 不再直接退出、映射到同一提示；composer 中有选区时 Ctrl+C 仍是复制（`ChatInput._on_key` 按 `selected_text` 分流）。克制的动效：busy spinner 帧同时驱动 header/activity/status panel，pending 审批卡以约 0.5s 脉冲切换 `-pulse` 边框。
- TUI busy 状态仍由 outbound 消息启发式推断（`chat_bot` 清除），turn 级结构化状态事件与 `expected_request_id` 审批原子校验尚未接入，属于已知遗留边界。
- 回归基线：`tests/tui_regression.py`（70 项 Textual headless Pilot 界面回归，含截图级日志抽屉重开、状态面板、设置滚动、模型页居中和双击退出断言）与 `tests/functional_regression.py`（25 项真实运行时 + 真实 DeepSeek API 控制面回归）已全绿；无 pytest 依赖，直接 `uv run python tests/<file>` 执行。真实 API 回归验证了 direct turn 应答与持久化；模型目录改动在 finally 中还原 config。
